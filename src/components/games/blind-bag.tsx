...
export function BlindBag() {
  ...
  const handleOpenBag = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setAnimationState('opened'); // Or 'idle' if 'opened' isn't visually distinct besides stopping shake
      setShowConfetti(true);
      setTimeout(() => setShowConfetti(false), 7500);
      setIsOpening(false);
    }, 3000); // 3 seconds of shaking
    ...
    <div className="space-y-6">
      ...
      <div className="relative flex flex-col items-center space-y-4">
        <div 
          ...
          style={{ perspective: '1000px' }} // For potential 3D effects if desired later
        >
          <Gift 
            className={cn(
        ...
        <Button onClick={handleOpenBag} disabled={isOpening || itemsList.length === 0} className="w-full max-w-xs py-3 text-lg">
          <Gift className="mr-2 h-6 w-6" />
          {isOpening ? translations.openingBagButton as string : translations.openBagButton as string}
        </Button>
      </div>
